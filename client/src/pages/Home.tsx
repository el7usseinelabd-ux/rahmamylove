...
function AccessShell({ children }: { children: React.ReactNode }) {
  return (
    <main dir="rtl" className="access-screen access-v3">
...
export default function Home() {
        ...
        <AccessShell>
          <section className="entry-stage">
            ...
            <div className="entry-card">
              <div className="entry-card-top"><span>ROO · PRIVATE LETTER</span><span>NO. 01 / 2026</span></div>
              <div className="entry-seal"><MarkSeal /></div>
              <p className="eyebrow">إلى رو — من غير زحمة</p>
              <h1>خدي نفس،<br /><em>وبعدين افتحي قلبي.</em></h1>
              <p className="entry-copy">دي مش صفحة عادية. دي ليلة صغيرة متخبية ليكي، فيها صورنا وكلام اتأخر كتير.</p>
  ...
  if (showWelcome) {
    return <><audio ref={audioRef} src={MUSIC_SRC} loop preload="metadata" autoPlay /><AccessShell><div className="welcome-card welcome-v3"><span className="welcome-petal petal-one">✦</span><span className="welcome-petal petal-two">♡</span><span className="welcome-petal petal-three">✦</span><div className="welcome-seal"><MarkSeal /></div><p className="eyebrow">الظرف الأول وصل</p><h1>أهلًا بيكي<br /><em>يا رو.</em></h1><p>خدي وقتك… كل تفصيلة هنا معمولة عشان تفتكري قد إيه إنتِ غالية.</p><button className="welcome-enter" onClick={() => setShowWelcome(false)}>افتحي الظرف <ArrowDownLeft size={17} /></button></div></AccessShell></>;
  }

  const scrollTo = (id: string) => document.getElementById(id)?.scrollIntoView({ behavior: "smooth" });

  return (
    <main dir="rtl" className="min-h-screen overflow-hidden bg-ink text-ivory selection:bg-berry/30 nocturne-2026">
      <audio ref={audioRef} src={MUSIC_SRC} loop preload="metadata" />
